...
param(
    ...
    [string]$Repository = 'open-cli-collective/salesforce-cli'
)

$nuspec = Join-Path $PackageDirectory 'salesforce-cli.nuspec'
...
if (-not (Test-Path $nuspec) -or -not (Test-Path $installScript)) {
    throw "Chocolatey package files are missing from $PackageDirectory"
}

$nuspecContent = Get-Content $nuspec -Raw
if (-not $nuspecContent.Contains('<version>0.0.0</version>')) {
    throw "$nuspec must contain the 0.0.0 package version before rendering"
}
Set-Content $nuspec ($nuspecContent.Replace('<version>0.0.0</version>', "<version>$Version</version>"))

$baseUrl = "https://github.com/$Repository/releases/download/v$Version"
$values = [ordered]@{
    URL_AMD64_PLACEHOLDER      = "$baseUrl/sfdc_${Version}_windows_amd64.zip"
    URL_ARM64_PLACEHOLDER      = "$baseUrl/sfdc_${Version}_windows_arm64.zip"
...
if ($content -match 'URL_(?:AMD64|ARM64)_PLACEHOLDER|CHECKSUM_(?:AMD64|ARM64)_PLACEHOLDER|ChocolateyPackageVersion|\$\{\s*version\s*\}') {
    throw "$installScript still contains an unrendered placeholder or runtime package-version expression"
}
